/* ============================================================
   ...
   function). Same pipeline:
     user message -> greeting/thanks shortcut
                  ...
                  -> compose a short reply with DeepSeek (optional;
                     falls back to a templated reply if LLM is down)
   Response shape:
     { message, localResults: Book[], webResults: Book[], sources[] }
   Keys come from Worker secrets/bindings (env), never shipped to the
   client.
   ============================================================ */
...
function searchExternal(query, env) {
  return Promise.allSettled([searchGoogleBooks(query, env), searchOpenLibrary(query)]);
}
...
async function composeWithLLM({ message, history, localResults, webResults }, env) {
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const [gb, ol] = await searchExternal(query, env);
      const sources = [];
      const rawWeb = [];
      if (gb.status === "fulfilled") { rawWeb.push(...gb.value); sources.push("Google Books"); }
      else console.error("Google Books search failed:", gb.reason?.message || gb.reason);
      if (ol.status === "fulfilled") { rawWeb.push(...ol.value); sources.push("Open Library"); }
      else console.error("Open Library search failed:", ol.reason?.message || ol.reason);
      const webResults = dedupe(localResults, rawWeb);

      const llmReply = await composeWithLLM({ message: rawMessage, history, localResults, webResults }, env);
